...
export default function FlashSaleEdit() {
  ...
  const { data: flashSaleById, isLoading: isLoadingById } = useQuery({
    queryKey: ["flashSaleAmin", id],
    queryFn: () => flashSaleApi.getFlashSaleById(id),
    keepPreviousData: true,
  });

  useEffect(() => {
    if (!flashSaleById && isLoadingById) return;
    setValue("name", flashSaleById.name);
    setValue("startTime", formatDateTime(flashSaleById.startTime));
    setValue("endTime", formatDateTime(flashSaleById.endTime));
    setValue("type", flashSaleById.type.toString());
    setValue("status", flashSaleById.status.toString());
    setDataProductSale(
      flashSaleById?.product_sales?.map((item, index) => ({
        number: index + 1,
  ...
  }, [flashSaleById, isLoadingById, setValue]);

  const { data: listProducts, isLoading: isLoadingProducts } = useQuery({
    queryKey: [
      "productsAllAdmin",
      { all: 1, type: 1, option, startTime, endTime },
    ],
    queryFn: () =>
    ...
    const formattedData = listProducts.products
      ...
      .map((item) => ({
        ...
        label:
          item.name +
          " - " +
          (item.discount ? formatVND(item.discount) : "Không có giá"),
      }));
  ...
  const mutationCreateFlashSale = useMutation({
    mutationFn: flashSaleApi.updateFlashSale,
    onSuccess: () => {
      toast.success("Chỉnh sửa chương trình thành công!");
    },
    onError: (error) => {
      const errorMessage =
  ...
  const onSubmit = (data) => {
    ...
    mutationCreateFlashSale.mutate({ id, data: newSale });
  };

  if(isLoadingById) return <BannerFormSkeleton />;
        ...
        <div className="grid-cols-1 gap-4 sm:grid md:grid-cols-2 mb-6">
          ...
          <div>
            ...
            <div className="bg-gray-100 p-5 rounded-sm space-y-3">
              <Select
                isDisabled={
                  isLoadingProducts || !startTime || !endTime || !option
                }
