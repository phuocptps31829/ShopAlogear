...
export default function CooperatesEdit() {
  const [image, setImage] = useState("");
  const { id } = useParams();
  const { data, isLoading } = useQuery({
    queryKey: ["cooperateById", id],
    queryFn: () => CooperatesApi.getCooperateById(id),
  ...
  const mutationEditCooperate = useMutation({
    mutationFn: CooperatesApi.updateCooperateAdmin,
    onSuccess: () => {
  ...
  const onSubmit = (data) => {
    ...
    mutationEditCooperate.mutate({ id, data: newCooperate });
  };
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="flex justify-end gap-3">
          ...
          <button
            disabled={
              mutationCreateImages.isPending ||
              mutationEditCooperate.isPending
            }
            type="submit"
